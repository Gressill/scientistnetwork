		...
		public function Relation(...args){
			...
			if(args.length == 0){		
				...
				if(args[0]){
					...
					//人数太多的话就缩小圆半径
					if(authorPaperdata.length>100){
						Constant.RADIUS = Constant.RADIUS-2;
					}if(authorPaperdata.length>100){
						Constant.RADIUS = Constant.RADIUS-2;
					}if(authorPaperdata.length>200){
						Constant.RADIUS = Constant.RADIUS-4;
					}if(authorPaperdata.length>300){
						Constant.RADIUS = Constant.RADIUS-6;
					}if(authorPaperdata.length>400){
						Constant.RADIUS = Constant.RADIUS-8;
					}if(authorPaperdata.length>500){
						Constant.RADIUS = Constant.RADIUS-10;
					}if(authorPaperdata.length>60000){
						Constant.ERROR_MESSAGE = "too many coauthors, more than 500!";
						Constant.RADIUS = Constant.RADIUS-12;
					}
...
//					var ay:Number = (ty - tempBall.y) * spring;
		...
		private function WherePoint(point:*) :int {
			...
			if((relationBalls[0].x - point.x ) >0 && (relationBalls[0].y - point.y) > 0){
				return 0;
			}
			if((relationBalls[0].x - point.x )<0 && (relationBalls[0].y - point.y) > 0){
				return 90;
